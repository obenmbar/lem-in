...
func (R *Room) AddRoom(Dataroom string) error {
	...
	if strings.Count(name[0],"-")> 0 {
		return fmt.Errorf("ERRor en Format de nom de Sommet , il ne doit pas contient '-'")
	}
	R.Name = name[0]
	
	R.X, err = strconv.Atoi(name[1])
	if err != nil {
		return fmt.Errorf("error en addroom , X n' est pas valide: %v", err)
	}
	R.Y, err = strconv.Atoi(name[2])

	if err != nil {
		return fmt.Errorf("error en addroom , Y n' est pas valide: %v", err)
	}
